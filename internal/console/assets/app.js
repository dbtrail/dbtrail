  ...
  if (c.baseline_configured) {
    if (c.full_table_status === "unknown") {
      ...
      if (c.unevaluable_tables && c.unevaluable_tables.length) {
        ...
        card.append(el("p", { class: "cov-line warn", text: "Their newest snapshot is older than the window this index can prove, and the archives cannot be tied to one source: " + c.unevaluable_tables.join(", ") + ". Take a fresh snapshot to settle it." }));
      }
    }
    if (c.full_table_from) {
      card.append(el("p", { class: "cov-line" },
        "Full-table restore for tables with a snapshot: any point from ",
        el("b", { text: c.full_table_from, title: utcLocalTitle(c.full_table_from) || null }), " onwards."));
    }
    if (c.broken_tables && c.broken_tables.length) {
      card.append(el("p", { class: "cov-line bad", text: "Not fully restorable (newest snapshot predates coverage): " + c.broken_tables.join(", ") + ". Take a fresh snapshot." }));
    }
    if (c.restore_needs_local) {
      card.append(el("p", { class: "cov-line warn", text: "Snapshots for this server go to S3 only, so \"Restore to a moment\" has no local folder to build into. Time-travel still reads them. Set this server's Local folder to restore here." }));
    }
    if (c.unreachable_tables && c.unreachable_tables.length) {
      ...
      card.append(el("p", { class: "cov-line warn", text: c.restore_reads === "s3"
        ? "Backed up only on this host, not in S3, so \"Restore to a moment\" (which folds from this server's S3 snapshots) cannot use them: " + names + ". Time-travel still reads them. Take a full read to send them to S3."
        : "Backed up only in S3, so \"Restore to a moment\" (which folds from this server's Local folder) cannot use them: " + names + ". Time-travel still reads them. Set this server's S3 location, or take a local snapshot, to restore them here." }));
    }
  }
  return card;
}
