...
const (
	assetVocabularyPin      = 234 // string literals in assets/app.js (includes the frozen permission name baseline:create, PERM_SNAPSHOT_CREATE, the frozen docs slug "guides/backup-strategy" each server block links since #1681 removed the card that carried it, and the frozen route "/api/baselines" the Overview flow reads for the copy arrow)
	goVocabularyPin         = 222 // string literals in this package's non-test .go files; #1681's frozen names (two import paths of internal/baseline) balance the refresh endpoint's it removed
