...
const ddlStreamState = `CREATE TABLE IF NOT EXISTS stream_state (
    ...
    dedup_floor_event_id BIGINT UNSIGNED DEFAULT NULL COMMENT 'lower bound on the event_id of every row the next resume-time dedup must delete (#1690), so that DELETE is a PRIMARY range instead of a full scan of binlog_events. Advanced at the same event that advances the coordinate this row persists: the statement/commit/DDL boundary in position mode, the commit in GTID mode. NULL = no floor, which is what an older build wrote and what --reset and the gap auto-advance stamp leave behind; both passes then run unbounded, exactly as before',
