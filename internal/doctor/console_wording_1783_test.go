...
// the index database gets created instead of which command creates it. A
// command-line spelling may still appear when the text says it is one ("on
// the command line"), which this pattern does not match.
...
func assertConsoleWording(t *testing.T, where string, c CheckResult) {
	...
	for field, s := range map[string]string{"detail": c.Detail, "remediation": c.Remediation} {
		if m := consoleOnlyWrong.FindString(s); m != "" {
...
func TestServerIDCollisionWording(t *testing.T) {
	...
	for name, srcID := range map[string]string{
		"no collision": "1",
		"collision":    strconv.FormatUint(uint64(derived), 10),
	} {
		t.Run(name, func(t *testing.T) {
			...
			defer db.Close()
			mock.ExpectQuery("SELECT @@server_id").WillReturnRows(sqlmock.NewRows([]string{"@@server_id"}).AddRow(srcID))
			c := checkServerIDCollision(t.Context(), db, dsn)
			assertConsoleWording(t, name, c)
		})
	}
	// A DSN that cannot be parsed never reaches the query.
	assertConsoleWording(t, "underivable", checkServerIDCollision(t.Context(), nil, "not a dsn"))
}
