...
CREATE OR REPLACE VIEW "state_legacy_db_audit_log" AS
  ...
  WHERE CASE WHEN (SELECT count(*) FROM glob('/data/baselines/current/Legacy-DB/Audit Log.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: Legacy-DB.Audit Log now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
CREATE OR REPLACE VIEW "state_shop_order_items" AS
  SELECT * FROM read_parquet('/data/baselines/current/shop/order_items.parquet')
  WHERE CASE WHEN (SELECT count(*) FROM glob('/data/baselines/current/shop/order_items.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: shop.order_items now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
CREATE OR REPLACE VIEW "state_shop_orders" AS
  ...
  WHERE CASE WHEN (SELECT count(*) FROM glob('/data/baselines/current/shop/orders.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: shop.orders now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
...
CREATE OR REPLACE VIEW "state_shop_order_items_2" AS
  ...
  WHERE CASE WHEN (SELECT count(*) FROM glob('/data/baselines/current/shop_order/items.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: shop_order.items now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
