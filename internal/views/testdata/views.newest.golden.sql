...
CREATE OR REPLACE VIEW "state_shop_orders" AS
  ...
  WHERE CASE WHEN (SELECT count(*) FROM glob(getvariable('bintrail_newest_snapshot') || 'shop/orders.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) + (SELECT count(*) FROM glob(getvariable('bintrail_newest_snapshot') || 'shop/orders.[0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: shop.orders now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
...
CREATE OR REPLACE VIEW "state_shop_order_items_2" AS
  ...
  WHERE CASE WHEN (SELECT count(*) FROM glob(getvariable('bintrail_newest_snapshot') || 'shop_order/items.[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) + (SELECT count(*) FROM glob(getvariable('bintrail_newest_snapshot') || 'shop_order/items.[0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9].upserts')) > 0 THEN error('bintrail views: shop_order.items now has a table delta beside its file, and this view reads the file alone, so it would show the table as it was when it was last written in full. Generate the views again') ELSE true END;
