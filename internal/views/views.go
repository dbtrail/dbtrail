...
type BaselineTable struct {
	...
	Delta bool
	// DeltaLegacy marks the v0.83.0 layout (one pair, no sequence): read with
	// the legacy state body while the snapshot is retained.
	DeltaLegacy bool
}
...
func MarkTableDeltas(ctx context.Context, tables []BaselineTable) error {
	bySnapshot := map[string]map[string]*baseline.TableDeltaChain{}
	for i := range tables {
		snap := snapshotDirOf(tables[i].Path)
		deltas, ok := bySnapshot[snap]
		if !ok {
			var err error
			if deltas, err = baseline.SnapshotTableDeltaChains(ctx, snap); err != nil {
				return err
			}
...
func writeHeader(b *strings.Builder, in Input) {
	...
	if in.Follow.follows() {
		...
		b.WriteString("-- again from the web interface) after a table is added or dropped, after a\n")
		b.WriteString("-- column changes type, and whenever archive sources are added or removed.\n")
		b.WriteString("--\n")
		...
		b.WriteString("-- file again from the web interface) after taking or refreshing a baseline,\n")
		b.WriteString("-- and whenever archive sources are added or removed.\n")
		...
		b.WriteString("-- again from the web interface) after taking or refreshing a baseline.\n")
	}
