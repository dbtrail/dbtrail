...
func MakeRecoverTool(cfg Config) func(context.Context, *mcp.CallToolRequest, RecoverArgs) (*mcp.CallToolResult, any, error) {
	return func(ctx context.Context, req *mcp.CallToolRequest, args RecoverArgs) (*mcp.CallToolResult, any, error) {
		...
		if dialect == recovery.MySQLDialect && recovery.RowsCanCascade(rows) {
			...
			case adv.AppliesTo(rows, args.Table):
				w = adv.Warning(args.Table, "the recover_cascade tool")
			}
			if w != "" {
				text += "\n-- Warning: " + w + ".\n"
				warnings = append(warnings, w)
		...
		if len(stmtEnds) != n {
			return ErrorResult(fmt.Errorf("internal error: %d reversal statement(s) rendered but %d statement offset(s) recorded; refusing to serve the script from this surface. Write it whole with `bintrail recover` from the CLI, and report this", n, len(stmtEnds))), nil, nil
		}
		script, serr := deliverScript("bintrail recover", text, stmtEnds, true, args.SummaryOnly, args.SQLOffset, args.SQLLimit)
		if serr != nil {
