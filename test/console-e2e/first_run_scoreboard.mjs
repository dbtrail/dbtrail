...
export const COLUMNS = {
  ...
  words_per_step_max: { label: "Visible words per step (worst step)", better: "lower", target: 40 },
  words_per_error_max: { label: "Visible words per error screen (worst)", better: "lower", target: 60 },
  ...
  primary_below_fold_px: { label: "Primary button below the fold at 1280x720 (px, worst)", better: "lower", target: 0 },
...
export function deriveRun(name, log) {
  ...
  const maxWords = (kind) => {
    ...
    if (!pick.length) return nm(kind === "error" ? "no error screen in this run" : "no screen was measured");
  ...
  const compute = {
    ...
    forced_choices: () => measured(of("forced_choice").length, { choices: of("forced_choice").map((e) => e.label) }),
    ...
    words_per_error_max: () => maxWords("error"),
    banned_words: () => {
...
export function compareRatchet(scoreboard, baseline, opts = {}) {
  const failures = [], loosened = [], notMeasurable = [], passed = [];
  ...
  eachColumn(scoreboard, runs, opts.only, (run, id, col) => {
    ...
    if (isNM(now) && isNM(was)) { notMeasurable.push(`${run}.${id}: not measurable (${cur.reason})`); return; }
    if (isNM(now)) { failures.push(`${run}.${id}: was ${fmt(was)}, now not measurable (${cur.reason})`); return; }
  ...
  return { failures, loosened, notMeasurable, passed };
}
