...
func TestResolveFoldSource(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"local is one generation behind but shares an older snapshot: the bucket",
			both, append(files(t1.Add(5*time.Minute), "orders"), files(t0, "orders")...),
			append(files(t1, "orders"), files(t0, "orders")...), "", "s3://bucket/abirds/"},
		{"listings arrive oldest-first: still compared on the newest",
			both, append(files(t0, "orders"), files(t1, "orders")...),
			append(files(t0, "orders"), files(t1, "orders")...), "", "/b"},
		{"listings arrive oldest-first and only the old one is shared: the bucket",
			both, append(files(t0, "orders"), files(t1, "orders")...),
			files(t0, "orders"), "", "s3://bucket/abirds/"},
