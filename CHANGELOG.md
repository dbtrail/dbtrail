...
- **A DROP or RENAME of several tables records every one of them.** Capture
  ...
  which row carries it, so `list_schema_changes` and the console list are not
  flooded by one long DROP), and a DROP or RENAME longer than the column holds
  is cut to fit instead of failing the whole record. `bintrail status` counts
  ...
  its real name; before, it was recorded as `a`.
