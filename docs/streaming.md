...
**How long that cleanup takes**: it is bounded by the checkpoint, not by the size of the index. Every checkpoint records the lowest `event_id` the next cleanup could possibly need to delete, and the delete only looks above it — a range on the primary key instead of a scan of the whole table. Measured on a 20,000-row, partitioned fixture: 20,002 rows touched without the bound, 201 with it; the GTID pass, which is the slower of the two, goes from 59,606 to 203. Two cases have no bound recorded and run the old full scan, once each: the first resume after upgrading, because the saved checkpoint predates the column, and the first resume after `--reset`, which discards the position the bound described along with the checkpoint itself. Both heal at the next checkpoint. An unfillable-gap auto-advance keeps its bound — the advance only ever moves forward, so a row at or beyond the new position was captured after the old one and still sits above it, and a daemon recovering from a purge is the last one that should also be handed a full table scan.

**What a restart looks like while that cleanup runs**: it is a delete over `binlog_events`, and on an index with no recorded bound it can take minutes. Capture does not start until it finishes, so the daemon is up but no events are arriving. It says so now: one log line when the step starts (naming the mode, the file and the position it is cleaning from, and that capture is blocked), and one when it ends, with how many rows it deleted and how long it took — including when it deleted none, which is the usual outcome after a clean stop. In GTID mode a third line separates the two passes, because the second one (the scan for open-transaction rows) can take longer than the delete before it. In the console, the server keeps its Start/Stop controls and its row shows `CLEANING UP` instead of `PENDING`; over the API the same fact is `monitor_phase: "resume_cleanup"` on `/api/servers`, and `phase` inside the `monitor` object on `/api/servers/{id}/monitor`. The `bintrail_stream_*` metrics now exist from before this step rather than after it, so a restart shows up as a scrape target reporting zeros instead of one that disappeared. Those zeros are what the shipped `BintrailNothingBecomingRecoverable` rule reads (`time() - bintrail_stream_last_flush_timestamp_seconds > 300`, `for: 5m`), so on a long cleanup it now fires about five minutes in rather than five minutes after the cleanup ends. The alert is right either way — nothing is becoming recoverable while the cleanup runs — but it arrives earlier than it used to, and the log lines and `CLEANING UP` phase above are what tell whoever it wakes that the daemon is working rather than stuck.
