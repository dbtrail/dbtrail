...
**Local-only folders keep the newest N instead (#1681).** A console server with a local folder and no S3 destination has a keep-newest count (`local_keep_newest` in the server list; a server added from the console gets 3, servers saved before #1681 have none and keep everything). `watch` prunes such a folder to that count on every cycle, whether or not `--baseline-retain` is set: the newest N complete snapshots that hold a table and are dated at or before now always stay, and so do the newest snapshot of each table, `_INCOMPLETE` snapshots, the `current` pointer's target and anything younger than an hour (or than `--baseline-retain`, when set). A folder that another server also sends to S3, or the daemon's own `--baseline-dir`, is left to the rule above. Each prune that removes anything writes `.last-prune.json` (when, and how many) beside the snapshots; the Snapshots page reads it to say why copies are gone. Two prunes of one folder never overlap: the second steps aside.
