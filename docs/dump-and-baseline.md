...
For audit or compliance purposes, you may want periodic full baselines. A weekly or monthly schedule is typical. From the web interface, set **full backup every** on the server's schedule (see above; its weekday is fixed by the grid, a Thursday for `7d`); to choose the day, use cron:
