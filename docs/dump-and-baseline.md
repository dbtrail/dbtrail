...
**Local-only folders keep the newest N instead (#1681).** A console server with a local folder and no S3 destination has a keep-newest count (`local_keep_newest` in the server list; a server added from the console gets 3, servers saved before #1681 have none and keep everything). `watch` prunes such a folder to that count on every cycle, whether or not `--baseline-retain` is set: for every table, the newest N complete snapshots holding it (dated at or before now) always stay, so a run over a few tables never pushes out the only other copies of the rest, and so do `_INCOMPLETE` snapshots, the `current` pointer's target and anything younger than an hour (or than `--baseline-retain`, when set). A folder two servers share, or the daemon's own `--baseline-dir` (compared after resolving symlinks), is never pruned to a count: a snapshot does not record which server wrote it. A save that would point a server with a count at a folder that already holds snapshots is refused, and a new server added over such a folder keeps everything. Each prune that removes anything writes `.last-prune.json` (when, and how many) beside the snapshots; the Snapshots page reads it to say why copies are gone. Two prunes of one folder never overlap: the second steps aside.
...
Minutes matter because this interval is, in practice, the ceiling on the freshness of reporting. A `state_<schema>_<table>` view sees the world as of the snapshot it was generated against, so at `12h` the tier answers yesterday's questions and at `15m` this morning's. **A views file generated once keeps up with this schedule on its own.** `bintrail views` writes state views that FOLLOW the newest snapshot — locally through the `current/` pointer, on S3 through the newest `_SUCCESS` marker — so each refresh becomes visible to a dashboard reading them, with nothing to regenerate. Two forms are pinned to one snapshot instead, on purpose: a file generated with `--pin-snapshot`, for reproducible analysis against a fixed instant, and the `views.sql` published beside each snapshot. Those stop moving when the next refresh lands, which is what they are for; regenerate them on this schedule if you want them current. Every generated file names in its header which of the two it is. A malformed interval refuses at startup; a daemon with nothing refreshable yet starts and says so, because every tick re-checks: servers added from the console later are picked up automatically, and refusing would mean a compose file carrying the interval could not boot a fresh install.
...
**From the console, per server.** On a daemon with a baseline feature on (`BINTRAIL_CONSOLE_BASELINE_TRIGGER=1` or `--baseline-refresh-interval`), the Snapshots page can set a schedule for one server (#1442): one of six intervals from 5 minutes to 24 hours (the daily one lined up on a UTC hour). The operator picks when; the daemon picks how, per run, and the page says which it will be: a server with no local backup directory gets a full backup, and so does one with no previous backup yet; otherwise the newest backup is updated from the recorded changes, this same refresh, with no load on the source. When the backups go to S3 that update reads its previous snapshot from the bucket and uploads its result back there, so an S3 destination does not force a full backup every slot. An update that fails (a capture gap, a schema change, too many changed rows, an internal error) falls back to a full backup at the same slot when the daemon may take one (the creation opt-in); otherwise that slot is recorded as skipped with both reasons. **A schedule can also ask for full backups of its own** (#1564): **full backup every** (for example `7d`, at least as long as **every**, empty for none) is a second timetable on the same fixed grid and the same UTC time. At each of its slots the run is a full backup from the source whatever the rule above would pick, so the backups stop resting only on the recorded changes, on the cadence you state rather than whenever an update happens to fail. When it is a whole multiple of **every** (`6h` with `7d`) each full backup lands on a scheduled run and takes that run's place; otherwise some fall between runs and are runs of their own. It needs the creation opt-in: a save asking for full backups on a daemon that cannot take one is refused with the reason, and when the opt-in is turned off later the page says so in red before the next one is due, each missed full backup is recorded as skipped with the reason, and the updates keep running. The schedule can still be edited without removing a full backup that was refused this way; setting a new one is refused (and a save that does not mention **full backup every** at all, from a script or a page loaded before an upgrade, keeps the one saved). A full backup slot that finds another job holding the server is not lost: the next scheduled run takes the full backup instead of waiting a whole **full backup every**, and the card says so while that is the plan. The debt lives in the daemon's memory, so a restart or a save of the schedule in between drops it; the miss then stays on the card until a full backup succeeds, and the next one is the timetable's own slot. Moving the schedule's UTC time moves every full-backup slot, so the timetable starts afresh from that edit. A slot that passed while the daemon was stopped is not made up (no read of production at boot), but it is recorded as missed when the daemon starts. A full backup that did not start, or started and failed, stays on the card in red until a full backup of the server succeeds after it, whatever started that one (the timetable, the **Read database now** button, or the fallback for a failed update): a missed run is made up by the next run, a missed weekly full backup by nothing for a week. The timetable's grid starts at the Unix epoch like the schedule's, which was a Thursday, so `7d` always lands on a Thursday at the schedule's UTC time; the page shows the date of the next one. The page shows the next full backup, and the 30-day count adds the full backups that fall between runs and says how many read the whole database. It is stored as `full_every` in the schedule. Every run publishes a full-table snapshot, and local-only backups are never removed automatically: the page shows the 30-day count next to the form and the daemon logs it at save and at boot. It lives on the server's registry entry as `backup_schedule` and the watch daemon reads it every minute, so it applies without a restart. Three rules keep it predictable: the timetable is a fixed grid (`every 1d at 03:00` is 03:00 UTC daily, `every 6h at 03:00` is 03/09/15/21; an interval that does not divide a day evenly drifts, and the page shows where it lands next), a time that passed while the daemon was stopped is not made up (and saving or editing a schedule never starts one on the spot), and a scheduled run that finds another backup job on the server skips that time rather than queuing. Every finished scheduled run and every skip lands in the baseline run history (a run that crashed writes no record; the daemon keeps its outcome for the page until its next run) (a streak of identical skips is one record whose time moves to the latest missed slot), so the page shows the last run, its result and the last skip, restart or not. A per-server schedule and the daemon-wide interval can run side by side; they share the one-job-per-server lock like everything else that produces a backup.
...
- **A limit on changed rows held in memory.** An update keeps one row image per changed row until it merges. The daemon (the refresh, a scheduled update, a restore to a moment, a `.sql` backup) keeps at most 2,000,000 of them in memory, divided by the tables folding at once. Past that, it writes that table's changes to the system temp directory in 64 groups and merges a few groups at a time, reading the backup it starts from once per set of groups. The snapshot is the same; it takes longer and uses temp disk for the changes, and a disk that fills while they are written is recorded as a disk refusal. It refuses, before publishing anything, only when one group alone passes the limit, which takes about 64 times as many changed rows (about 64,000,000 of one table with two tables folding at once). A build of a table that has no backup at all, rebuilt from the recorded changes alone, still refuses at the limit itself. Then a full backup, which reads the table instead of holding its changes, is the way out, and a restore needs a moment closer to an existing backup. The limit is per run, and each server runs its own, so several servers updating at once each get it. CLI runs have no limit and keep every change in memory.

  **Why the merge is not handed to DuckDB.** DuckDB can merge two tables and spills its own joins to disk, so it looks like the natural tool. But the recorded changes and the backup spell a row's key differently: the index stores the key as text written by the capture (a `DATETIME` as the binlog parser prints it, a `BINARY(n)` without its trailing zero bytes), while the backup stores the typed values `mydumper` dumped. The Go code that turns one spelling into the other, per key type, is what earlier bugs with date keys, binary keys and a row stored under two spellings were fixed in. The changes' values need Go too (`ENUM` numbers and base64 `BLOB`/`TEXT` are decoded against the schema in effect when each change happened). Grouping the changes on disk in Go keeps both translations as they are and changes only where the changes wait. The full reasoning is at the top of `internal/reconstruct/changespill.go`.
- **The events are read in index order.** A refresh reads each table's window off the index's `(schema, table, timestamp)` entry in the order it folds them, oldest first, page by page, and picks up each row as it goes. It does not do what the other readers do, which is to sort the page's keys first and then fetch each row by primary key: under load, with the index's pages pushed out of memory by the capture, that fetch was one disk read per row (#1720, about 1.7 ms each and 100,000 per page). On an index written by `bintrail stream` or the daemon, each file a refresh writes records the highest event id it applied, and the next refresh reads no row below that id. The scan still starts the hour before its anchor, the safety margin for a transaction that executed before the anchor and committed after it; the id only lets the rows of that hour the previous refresh already applied be skipped in the index instead of read. An index built with `bintrail index --files` gets neither the stamp nor the floor: there, ids follow the order the files were given, which need not be the binlog's. Every per-table log line (`table reconstructed`, `table carried forward unchanged`, `table published as a delta` and the rest) carries `fetch_ms` and `fold_ms`, so a slow table says whether it waited on the index or on the fold, including a table that waited and changed nothing.
...
- **A table with no changes is left as it is (on by default since #1681).** When the delta window held no events for a table, its previous Parquet file is published into the new snapshot as-is (a hard link where the filesystem allows one, a copy otherwise) instead of being folded and re-emitted. It does not apply when the previous snapshot is read from S3: linking a file needs both ends on a filesystem, so those runs take the ordinary path and the daemon log says so. A per-server schedule on an S3-backed server reads the bucket, with one exception (#1626): when the server's local directory already holds the bucket's newest snapshot, table for table, which is the normal state after the first run (the fold writes locally and uploads, and retention never removes the newest snapshot), the run reads that local copy and unchanged tables are reused; an older or partial local copy keeps the bucket. Each table's file carries its OWN binlog anchor, so an untouched table's anchor still points exactly where its deltas resume. The rows are identical either way; the on-disk representation is not: two snapshots end up sharing one file, so the prune pass reports space it will not reclaim while the newer snapshot still references it (a `du` run per snapshot directory double-counts it too; one `du` over the baseline root reports the truth). That is a reporting cost, not a correctness one, which is why it is on by default: a destructive DDL or a stale schema snapshot refuses the backup before reuse is reached; a known capture gap (including under `--allow-gaps`, which reports and proceeds) makes that table ineligible, so it is folded and written the ordinary way; and a failed `_MANIFEST` check on the file being reused fails the run rather than copying a corrupt file forward. The run summary calls a reused table `unchanged` rather than `refreshed`, and the console counts them separately, because which tables actually cost a full rewrite is the number worth seeing. A `TRUNCATE`, `DROP` or `RENAME` emits no row events either, which is why the destructive-DDL refusal runs first and this can never republish rows a truncate deleted.

  | Where | How |
  |---|---|
  | `bintrail baseline refresh` | `--carry-forward-unchanged` (opt-in on this one-shot command) |
  | `bintrail reconstruct --output-format parquet` | `--carry-forward-unchanged` (opt-in) |
  | `bintrail-console watch` | on by default; `--baseline-carry-forward-unchanged=false`, or `BINTRAIL_BASELINE_CARRY_FORWARD_UNCHANGED=false` (a true/false value: `1`, `true`, `0`, `false`; anything else keeps the default, which is now ON), turns off THIS path — with table deltas on (the default, below) the previous file is still linked forward, so writing every table again needs both off |
  | Console | nothing to set (#1681); a setting saved by an older console is ignored. The saving only applies to a server that keeps a copy on this machine, which is the per-server question under **Where and how often** on the Snapshots page |

- **A table that changed is left as it is too, with its changes written beside it (on by default since #1729; #1638, #1718).** Turned off, a refresh rewrites every table that changed in full, however little of it changed. On, the table's previous Parquet file is published into the new snapshot untouched (a hard link, as above) and each refresh writes ONE new pair of small files next to it, numbered in sequence:
  ...
  <snapshot>/<schema>/<table>.000001.posdel    refresh 1: row numbers of rows in the table file that this refresh replaced or removed
  <snapshot>/<schema>/<table>.000001.upserts   refresh 1: the rows it changed or added, and a tombstone for each row it deleted
  <snapshot>/<schema>/<table>.000002.posdel    refresh 2, and so on
  <snapshot>/<schema>/<table>.000002.upserts
  ```

  All the small files are Parquet; they carry another suffix so that nothing that lists `.parquet` files can take one for a table. The table's state at the snapshot is the table file minus every row number named in any `.posdel`, plus, for each key that appears in the `.upserts` files, its newest version, unless that version is a tombstone. A `.upserts` row carries two technical columns besides the table's own: `bintrail_pk`, the row's key as one string, and `bintrail_op`, `u` for a current row or `d` for a tombstone. A refresh links every earlier pair into the new snapshot and writes only its own: what it writes grows with the rows changed since the previous refresh, never with the chain (the layout in v0.83.0, one pair per table rewritten on every refresh, cost 28 µs per accumulated row per refresh under load, and the cost grew until the chain ended; see #1718). A refresh whose window touched nothing writes no pair.

  The table is written again in full, and the chain starts over with an empty `000000` pair, when the chain's files together pass a quarter of the table's file (tables whose chain is under 1 MiB are left alone), when the chain is a day old, when a window's changes did not fit in memory, when the run went over a known capture gap, when the previous snapshot is read from S3, when the table has a column named `bintrail_pk`, `bintrail_op`, `filename` or `file_row_number`, or when the previous snapshot was written by v0.83.0 (its one pair is folded in, once). A schema change refuses the refresh exactly as it does without this option, and the full backup that follows starts a new chain. With the option on, every table in the snapshot has at least the `000000` pair.

  **A long chain is merged by the daemon (#1723).** Once a chain lists 16 entries (16 plain pairs the first time; after that, a range plus 15 pairs), `bintrail-console watch` runs a compaction job right after the refresh has released the server's slot, never inside the refresh's own time: a DuckDB-only merge of all but the last pair into ONE range pair, `<table>.000000-000014.posdel` / `.upserts` (the union of the dead positions; the newest version of every key, tombstones included), whose footer is the last merged pair's with the two ends added. The result waits under `<backup dir>/.compact/<schema>/<table>/<chain start>/` and the next refresh links it forward in place of the pairs it merged, so from that snapshot on the chain reads `000000-000014, 000015, 000016, ...` while older snapshots keep their plain pairs; the state read through the range is the state read through the pairs, and every reader (the views, the console, `verify`, restore) takes a range like any pair. The table file is never touched by this job. Every input pair is checked against the snapshot's manifest first; a result the refresh cannot adopt (another chain or base, footers that do not name its range) is removed with a warning, an unfinished one (a daemon that died mid-merge) is swept at the next refresh cycle, and a refresh that finds none carries the chain forward as before. The job shares the server's single-flight with every other backup job. It is recorded in the daemon log and in the on-disk run history file (`kind: compact`; a failure that repeats is one record whose end moves); it does not appear on the Snapshots page, which lists the runs that produced a snapshot, and a daemon shutdown mid-merge is logged, not recorded. A snapshot holding a range pair must not be read by a bintrail older than the one that wrote it.

  **Who reads the chain.** The generated DuckDB views do: a `state_<schema>_<table>` view over such a table reads the table file and every pair of the chain, so it shows the snapshot's rows. A query through it costs more than a query over one file, because the big file is read with its row numbers; on a 40M-row table a full-table sum went from 0.03 s to 0.12 s in the measurement on #1638. Everything else (`reconstruct`, `recover-cascade`, the shim's `_snapshot`, `verify`, `drill`, a restore, the Iceberg export) keeps reading the table file and the index, and stays correct because the index still holds every change since that file was written. Those readers fetch a longer window for such a table, up to the age of its chain, which is why the chain ends after a day. `verify` compares table files: for a table whose newest backup is its previous file plus a pair there is nothing for it to compare, so it reports that table as `inconclusive` and says why, and verifies it again the next time the table is written in full. The backup-age verdict in `status` and on the console is graded on the snapshot's time, not on the start of the chain, so with a retention shorter than a day it can read `ok` for a table those readers can no longer rebuild; they refuse with a coverage error in that case, they do not return wrong rows.

  **Two things to know.** A snapshot written this way must not be read by a bintrail older than the one that wrote it: an older build takes the snapshot's directory time as the start of its event window, and for a table with a pair the window has to start where the chain did, so it would skip changes without an error. And the chain has to travel with its table file. The daemon's own upload and `bintrail upload` both carry it, and the snapshot's `_MANIFEST` covers it; a copy made some other way that takes only `*.parquet` leaves a table file that is older than its directory says, and a reader of that copy skips changes without an error. Uploads still send the table's file and every pair again under each new snapshot prefix, so this saves local writes, not upload. If a pair is lost or damaged, at any sequence, the refresh sets the whole chain aside and rebuilds from the table file, and the other readers keep working from the time that file was written. Views generated by v0.83.0 that follow the newest snapshot guard against a `<table>.upserts` appearing beside a table, not against the numbered files, so generate them again after upgrading.
  ...
  **Turning it off needs nothing else.** The next refresh reads each table file and the index, writes every table in full and writes no pair, and the snapshot after it has the plain layout again. **Generate the DuckDB views again after turning it on, and again after turning it off.** A view's shape is fixed when it is generated. Views generated while it was off read the table file alone, and once tables have a pair that file is the table as it was when the chain started, with no error to say so. Views generated while it was on name the pair and fail once it is gone. The `views.sql` published inside each snapshot, the console download and the console SQL panel are generated per snapshot and are always right.
  ...
  | Where | How |
  |---|---|
  | `bintrail baseline refresh` | on by default; `--table-deltas=false` turns it off |
  | `bintrail-console watch` | on by default; `--baseline-table-deltas=false`, or `BINTRAIL_BASELINE_TABLE_DELTAS=false`, turns it off (a value that is neither true nor false keeps the default) |

  **Upgrading from a build where it was off** needs nothing: the next refresh starts a chain beside each table that changed (the table file is kept, an empty pair 000000 and this refresh's pair are written) and the daemon log says so once per table; a snapshot from v0.83.0's single-pair layout is compacted once with its changes, as above. Generate the DuckDB views again, for the reason below.
...
It shares its single-flight with the console's **Create baseline** button, so a refresh and a dump never run against the same server at once. The last outcome per server shows up on the console's **Snapshots** page and in `GET /api/baselines`.
...
**When the source was last read, inherited too** ([#1570](https://github.com/dbtrail/dbtrail/issues/1570)). A refresh chain can go a long time without reading the source, and nothing in one snapshot used to say how long. Every table file now carries `bintrail.last_dump_at` (the instant of the newest real dump its rows descend from; a dump stamps its own) and `bintrail.fold_generation` (how many refreshes built the rows since: 0 for a dump, and for a refresh one more than the file it started from). A refresh copies both from what it folded, so a descendant never looks fresher than its ancestor. The date never moves forward without a dump; the count can go down, because a refresh that starts again from an older file (table deltas turned off, a chain set aside as damaged, a previous backup read from S3) counts from that file. With table deltas the count is on the newest pair of the chain beside the table, since the table file under a chain is carried forward unchanged. A file carried forward is a hard link to the older snapshot's file and cannot be stamped; its values are that snapshot's, read where they are. A dump written before this reads as its own instant with zero refreshes; a refresh written before it cannot be dated, and neither can anything refreshed from it, until the next dump. The Snapshots page shows the oldest of these per snapshot, and the most refreshes since only when every table records its count (a chain written before this version keeps an unknown count until the next full backup, and the page says how many tables that is). It matters for `verify`: its default rebuilds one snapshot forward to the next, which proves much less when both ends were themselves built from recorded changes.
...
For audit or compliance purposes, you may want periodic full baselines. A weekly or monthly schedule is typical. Set `full_every` on the server's schedule through the schedule API (the page no longer edits it; see above; its weekday is fixed by the grid, a Thursday for `7d`); to choose the day, use cron:
