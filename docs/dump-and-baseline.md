...
- **A limit on changed rows.** An update holds one row image per changed row in memory until it merges, so the daemon refuses one (the refresh, a scheduled update, a restore to a moment, a `.sql` backup) once a table's distinct changed rows since the backup it starts from pass 2,000,000 divided by the tables folding at once, before writing anything. A full backup reads the table instead of holding its changes, so it is the way out; a restore needs a moment closer to an existing backup. The limit is per run, and each server runs its own, so several servers updating at once each get it. CLI runs have no limit.
