...
1. **Overview** (landing) — what changed recently and where. While the
   ...
   it: create the index database, read the table structure, start capturing
   changes, capture the first change, and a first backup when the console can
   create one for that server. Each step is waiting, running, done or failed;
...
| `GET /api/servers/{id}/first-run` | Supervisor only, servers with a source: `{complete, steps: [{name, state, detail, fix}], check_error}`, the Overview's Getting started list. `state` is `waiting\|running\|done\|failed`. Each capture step is done from the server's own index database (it exists, a schema snapshot, a saved stream position, `events_indexed > 0`); the first step not done takes the supervisor's state. `complete` is true once a change is indexed. A first-backup step is included only when console backups are enabled and the server has a baseline location. `check_error` means the index database could not be read, and nothing is marked done from it. |
