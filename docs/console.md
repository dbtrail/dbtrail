...
- **Backups & disk space** (read-only since #1681) — what DBTrail does with a
...
- **Backups & disk space** (#1528/#1543, formerly *File reuse for unchanged
  ...
  **process-global** (`GET /api/baseline-refresh`, read-only) even though it
  ...
  #1681 the console does not edit it**: reuse is correct wherever it runs (a
  destructive DDL, a capture gap or a failed `_MANIFEST` check refuses the
  backup before reuse is reached), so it is on for every daemon and
  `--baseline-carry-forward-unchanged=false` is the only way off. A
...
| `GET /api/baseline-refresh` | What the daemon does with a table that did not change: `{carry_forward_unchanged, enabled, scheduled, targets, skipped_s3_only}`. Read-only since #1681 (the `PUT` and the `source` field are gone): `carry_forward_unchanged` is the daemon's own flag, on unless it was started with `--baseline-carry-forward-unchanged=false`. `enabled` reports whether anything in this daemon consumes it, `scheduled` whether a refresh timer runs, `targets` how many servers the next tick covers (omitted where no loop runs) and `skipped_s3_only` how many it skips for keeping backups only in S3. |
