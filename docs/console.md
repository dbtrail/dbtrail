...
(the console's own or the session's) the page does not offer it. The page
learns the location from `GET /api/baselines?location_only=1`, which reads
neither the storage nor the server's index; when that lookup fails for any
reason other than a refusal, a one-line note takes the panel's place.
