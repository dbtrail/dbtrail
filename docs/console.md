...
1. **Overview** (landing) — what changed recently and where. While the
   ...
   the console can create one for that server. Each step is waiting, running, done or failed;
...
| `GET /api/servers/{id}/monitor` | Supervisor only: `{monitor: {state, last_error, since, source_connected}}` — `stopped\|pending\|running\|stalled\|lost_position\|failed`. |
| `GET /api/servers/{id}/first-run` | Supervisor only, servers with a source: `{complete, steps: [{name, state, detail, fix}], check_error}`, the Overview's Getting started list. `state` is `waiting\|running\|done\|failed`. Each capture step is done from evidence: the server's own index database exists, the supervisor reports `source_connected` for the current run, a schema snapshot (MySQL only), a saved stream position, and a change in the index; a later step's evidence marks the earlier ones done, and the first step not done takes the supervisor's state. `complete` is true once a change is indexed. A first-backup step is included only when console backups are enabled and the server has a baseline location. `check_error` means the index database could not be read, and nothing is marked done from it. |
